...
Vendor logs (Entra, Okta, PAN-OS, Windows, Wiz)
        │
        ▼  pipeline.py (OCSF Transformer → FusionGraph)
┌───────────────────────────────────────────┐
│  Neo4j Fusion Graph                        │
...
```
combined_score = 0.6 × autoencoder_score + 0.4 × isolation_forest_score
```
...
- **Threshold** — contamination-informed, set at `(1 - attack_rate × 1.3) × 100th` percentile
